...
- **100% measurability coverage, 0 frame-blind, 0 classifier-blind entries.** This means **Phase 7 (frame-coverage audit) has no upgrade candidates for the 2026 LLM cycle** — it is a no-op unless a future cycle produces frame-blind entries.
...
- **Selection bias test produced NaN** for both statistic and p-value (`selection_bias.json`). This is because Kruskal-Wallis requires >1 group with variance, and the single-stratum design after merging yields degenerate groups. Severity correctly defaults to "low". Not a bug — the test is structurally inapplicable to the current corpus design.
...
**Status:** NO-OP FOR 2026 LLM CYCLE — optional; only runnable if frame-blind entries exist.
...
- **CRITICAL: The 2026 LLM cycle has 0 frame-blind entries and 0 classifier-blind entries** (100% measurability coverage per `results/report.md`). Phase 7 exists to audit frame-blind entries and try to upgrade them — if there are none, there is nothing to audit. **The plan-creation prompt below must begin with a feasibility check: read `results/report.md` and count frame-blind entries. If zero, Phase 7 is a documented no-op for this cycle, not a failure.**
- There is no standalone `measurability_map.json` file. Measurability is computed at runtime from calibration posteriors (`calibration/posteriors.json`) and reported in `results/report.md`.
...
```
/using-superpowers Read docs/HANDOFF.md (focus on §4 Frame-coverage-audit row, §6 control 7, §9 item 9, §3 F-frame critical paragraph) and docs/PRD.md §8 (Plan 7). FIRST: read projects/owasp-llm/cycles/2026/results/report.md and check the "Frame-blind" line in the Measurability Map section. If 0 frame-blind entries exist, Phase 7 is a documented no-op for the 2026 LLM cycle — write a short note to docs/superpowers/plans/YYYY-MM-DD-frame-coverage-audit.md explaining this and skip plan creation. If frame-blind entries DO exist, proceed: read the Plan 5 closure notes in claudedocs/IMPLEMENTATION_PROMPTS.md, read docs/METHODOLOGY-CHANGELOG.md through current, git log v0.4.0-plan4..v1.1.0-plan5 --stat. For each frame-blind entry, determine if an external reference list (incidents NOT sourced from CVE or harm databases) is feasible — if not, that entry stays unmeasurable. Do not re-brainstorm HANDOFF or PRD; do not modify either. Invoke the Superpowers writing-plans skill and write the per-task plan to docs/superpowers/plans/YYYY-MM-DD-frame-coverage-audit.md. The plan MUST start with an "Inherited constraints from Phases 1-5" section listing: (i) no audit on a weak reference list — refusal is the honest outcome; (ii) per-entry scope, per-entry tags admissible; (iii) measurability map regeneration is the engine's job, the audit only produces the per-entry bound + uncertainty inputs; (iv) the staged audit is a declared extension, not a primary deliverable. Target tag: v1.3.0-plan7 (or per-entry sub-tags). --ultrathink
```

### Plan-execution prompt

```
/using-superpowers Read docs/superpowers/plans/YYYY-MM-DD-frame-coverage-audit.md. If the plan is a documented no-op (0 frame-blind entries), update PRD phase-map to "N/A — 0 frame-blind entries in 2026 cycle" and stop. Otherwise: confirm the "Inherited constraints from Phases 1-5" section is present, especially the "no audit on a weak reference list" criterion. Invoke the Superpowers executing-plans skill. For each candidate entry, the first task is reference-list feasibility — if infeasible, mark as REFUSED and document why. Commit per audited entry with messages tagged (Plan 7, entry <id>). Bump methodology-changelog, tag per-entry sub-tags. Update PRD phase-map. --ultrathink
```
