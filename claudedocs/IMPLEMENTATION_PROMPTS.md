...
- **Concordance is low:** kappa 0.275 [-0.01, 0.57]. Four entries flagged: LLM01 and NEW-PMP (vote_over_ranks), LLM05 and LLM09 (vote_under_ranks). This is an honest finding, not a bug — the taxonomy's incident-frequency ranking simply doesn't match the community's importance ranking well. Phase 6 corpus B corroboration should interpret agreement/disagreement against this baseline. (Note: kappa improved from 0.228 after fixing a Stage-2 hallucinated entry ID ROLL-WLA → ROLL-CFAS that had misaligned the entry-vote mapping.)
...
- `projects/owasp-llm/cycles/2026/results/report.md` — kappa 0.275, 4 flagged entries. Corpus B corroboration should be interpreted against this low-concordance baseline.
...
- **Plan 5 concordance lesson:** kappa 0.275 means incident-frequency ranking and community-importance ranking weakly agree. Corpus B agreement/disagreement sits on top of this already-weak signal — interpret accordingly.
