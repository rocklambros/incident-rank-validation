...
- **Inherited from Plan 1 v5.1 erratum (CI verification gap):** if Plan 2 adds or modifies CI (likely, given new adapter tests + snapshot scripts), the plan MUST include a task that verifies CI *actually executes the new logic to completion in a green run*, not merely that the workflow file parses with `actionlint` or that local tests pass. Plan 1 claimed CI-green acceptance for criteria 6/7/8/9 against a workflow that never executed a single job since it landed (YAML flow-style `}` collision suppressed all runs). The erratum is in `docs/METHODOLOGY-CHANGELOG.md` "Plan 1 v5.1 erratum (2026-05-20)". Five distinct CI bugs hid behind the first; every one would have been caught immediately if anyone had clicked the Actions tab once. Build that verification into the phase's definition of done.
