...
**Legibility budget (governs the method + width choice).** On-page point size ≈ `source_font_pt × (target_width_in / figsize_width_in)`, where `target_width_in = width × 6.5`. Target **≥8 pt on-page** for the smallest text in every figure. Consequence: a chart authored at a normal ~8-in figsize renders ~3–4 pt at a 0.42 wrap width — illegible. So **`wrap` is reserved for genuinely simple charts (≤ ~6 marks/labels), re-authored at a small figsize (~3.2 in wide) with large relative fonts** so the scale factor stays ≈1. Any multi-row / densely-labelled chart is **`center`**, sized for legibility. Widths below already reflect this.
...
| 5 | precision_bars | §3.4 | center `[htbp]` | 0.60 | 20 y-labels — too dense to wrap (F3) |
...
Wrap side alternates L/R for rhythm where prose allows; default `right`. Placement policy: center figures use `[htbp]` (avoids the forced-whitespace gaps `[H]` causes — F5); numbering stays monotonic because captions sit adjacent to their prose and the full-build pass checks order (no prose cross-references "Figure N", so mild drift is cosmetic only). Downgrade a specific figure to `[H]` only if it drifts far from its mention.

## 5. Redesign specs (all in `engine/report/narrative_charts.py`; signatures preserved)

**Common:** keep each render function's signature and output path (tests assert on these). Reuse `ENTRY_COLORS`. **Apply the §4 legibility budget:** for each figure, pick `figsize` + font sizes so the smallest on-page text is ≥8 pt at the target width. Wrap figures are authored *small* (~3.2 in wide) with large relative fonts (scale factor ≈1); center figures are authored larger. Compute the on-page pt for the smallest label and confirm ≥8 before sign-off.
...
- Left axis = 2025 published order, rows 1–10, label `N  LLMkk  <2025 name>`. Right axis = 2026 blended order, label `LLMjj  <2026 name>  (<move>)`, with `[renamed]` appended.
  - New right-side code `LLMjj` = blended position (1→LLM01 … 10→LLM10).
  - `<move>` = `+n` / `-n` / `nc`.
  - **`[renamed]` is driven by an explicit set `RENAMED_2026 = {"LLM07"}`, not a name string-diff** (F6) — a diff would over-tag cosmetic differences like "Supply Chain" vs "Supply Chain Vulnerabilities". Only LLM07 (System Prompt Leakage → Hidden Context Exposure) is a true rename, matching the reference screenshot.
...
2. **Pandoc Lua filter** `notebooks/preprint/figure-layout.lua`:
   - **Target `Figure` nodes, not `Para`/`Image`** (F1 — verified: pandoc 3.9 wraps every attributed image in a `Figure` AST node; a `Para`/`Image` filter no-ops and nothing wraps). Define `function Figure(fig)`: locate the descendant `Image` in `fig.content`, read `width`/`wrap` from **the Image's** `attributes`, and read caption inlines from **`fig.caption.long`**.
   - **Render the caption through pandoc, never as raw text** (F2 — verified: two captions contain `%` which would comment out the caption tail under raw `\caption{}`, and captions carry `λ/κ/ρ/–` needing the unicode path). Build the caption LaTeX with `pandoc.write(pandoc.Pandoc({pandoc.Plain(caption_inlines)}), 'latex')`, strip the trailing newline, and inject that string into the environment. This preserves pandoc's escaping + xelatex unicode handling.
   - If `wrap` attr present → `RawBlock('latex', "\\begin{wrapfigure}{R|L}{(w+0.03)\\textwidth}\\centering\\includegraphics[width=w\\textwidth]{src}\\caption{<rendered>}\\end{wrapfigure}")`.
   - Else → `\begin{figure}[htbp]\centering\includegraphics[width=w\textwidth]{src}\caption{<rendered>}\end{figure}` (F5 — `[htbp]`, not `[H]`, to avoid forced whitespace).
   - `w` parsed from the Image `width=NN%` attr; wrap side from `wrap=left|right` (unknown value → `log()` a warning and treat as center, so a typo surfaces instead of silently no-op). Both branches carry `\caption`, so the shared figure counter numbers every figure in source order; monotonicity confirmed in the build pass.
...
- **Cell 24:** delete the `render_plotly_rankings(...)` call cell (or blank it). Also remove `render_plotly_rankings` from the cell-0 import block so no dangling import remains (F8 — verified safe: cell 46 consistency guard has no dependency on it).
...
1. **Fast loop (correctness, not print-legibility):** standalone harness regenerates each changed PNG; read each to confirm *content* correctness. The harness **reuses cell-45's blend/fold code verbatim** for the slopegraph so its moves match the real build (F7); the PNG read confirms shape/colour but **cannot judge print-scale legibility** (F4). Present the 6 redesigned charts to the user (preview gate, §5).
2. **Full build + print-scale legibility check:** run `build_preprint.py` with the new output name, then **rasterize the PDF** (`pdftoppm -r 150` or `pdf2image`) and inspect pages at 100% print scale. Confirm:
   - figures renumbered 1–18; caption numbers monotonic;
   - §4.2 shows the slopegraph, **no table**;
   - **zero** figures occupy a full page **and** no page has a large forced-whitespace gap (F5);
   - every figure's smallest text is legible at print scale — the real test for F3 (the PNG loop can't see this);
   - wrapped figures flow cleanly (no wrapfig collisions near headings/lists);
   - filename correct; `.tex` emitted and re-compiles.
3. Run `pytest tests/unit/test_narrative_charts_new.py tests/unit/test_preprint_build.py` and the full suite.

## 11. Risks / mitigations

Hardened by the 2026-07-02 adversarial premortem (findings F1–F8; F1/F2/F8 verified against the toolchain). Top residual risks:

- **Print-scale illegibility (F3, highest).** Wrap widths imply ~3–4 pt text unless charts are re-authored small. → §4 legibility budget + §5 per-figure figsize/font pass; **verified on the compiled PDF at print scale (F4), not the PNG loop.**
- **Lua filter no-op (F1).** Must target pandoc-3.x `Figure` nodes. → §6.2 rewritten; build must show ≥1 `wrapfigure` in the `.tex`.
- **Caption compile break (F2).** `%` in two captions + unicode. → captions rendered via `pandoc.write`, never raw `\caption{}`.
- **`[H]` whitespace vs numbering reorder (F5).** → `[htbp]` default; monotonic order confirmed in the build pass (no prose "Figure N" refs, so drift is cosmetic).
- **Slopegraph preview ≠ build (F7).** → harness reuses cell-45 code verbatim; final build authoritative.
- **arXiv recompilation** → `wrapfig` + `float` are standard TeXLive (present locally and on arXiv).
