...
- Bayesian diagnostics within MCSE: R-hat < 1.01, ESS gate covers λ and σ_u, **divergences == 0** (not "or documented" — any divergence raises `DiagnosticsFailure` and halts), PPCs; determinism reproduces **within declared cross-pod MCSE** (threads/XLA pinned). Robustness specs apply the **identical** R-hat < 1.01 / ESS ≥ ess_fraction / divergences == 0 gate via the shared `_check_diagnostics` function (`engine/model/inference.py`); any violation writes `robustness_<spec>_failure.txt` and re-raises — NOT `diagnostics_failure.txt` (primary result is unaffected). This closes the previously-undisclosed "or documented" carve-out for robustness-spec divergences (U2-5 §14 disclosure).
