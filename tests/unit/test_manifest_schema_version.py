...
def _base_manifest(
    ...
    goldset_hash: str | None = None,
) -> PreregManifest:
    """Build a minimal manifest with all required fields; schema_version and
    goldset_hash are the only axes varied by these tests."""
...
def test_v1_manifest_excludes_new_fields_from_hash() -> None:
    m = _base_manifest()  # schema_version defaults to 1
    d = m.to_dict()
...
def test_adding_goldset_hash_under_v1_does_not_change_hash() -> None:
    ...
    m_plain = _base_manifest()
    m_with = _base_manifest(goldset_hash="abc123")
    assert compute_lock_hash(m_plain) == compute_lock_hash(m_with)


def test_v2_manifest_includes_new_fields_and_changes_hash() -> None:
    m1 = _base_manifest()
    m2 = _base_manifest(schema_version=2, goldset_hash="abc123")
    d2 = m2.to_dict()
